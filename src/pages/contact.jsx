...
//                       isCaptchaValid
//                         ? "bg-[#006d2c] hover:bg-[#005a24] cursor-pointer shadow-lg"
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://formsubmit.co/ajax/tribikram.infoxit@gmail.com",
        { method: "POST", body: formPayload }
      );
      if (response.ok) {
        setIsSubmitted(true);
        ...
        recaptchaRef.current?.reset();
        setIsCaptchaValid(false);
      } else alert("Failed to send message. Try again later.");
    } catch {
      alert("Something went wrong. Try again later.");
    }
  };
